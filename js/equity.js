...
(function() {
  var SUITS = ['\u2660', '\u2665', '\u2666', '\u2663'];
  var RANK_ORDER = ['2','3','4','5','6','7','8','9','T','J','Q','K','A'];
  ...
  function buildDeck() {
    ...
    for (var r = 0; r < RANK_ORDER.length; r++) {
      for (var s = 0; s < SUITS.length; s++) {
        deck.push(RANK_ORDER[r] + SUITS[s]);
      }
    }
    return deck;
  }
  ...
  function shuffleDraw(deck, n) {
    // Shuffle and return first n cards (modifies deck in place)
  ...
  function simulateStreet(heroHole, knownBoard, iterations) {
    ...
    if (knownBoard.length === 5) {
      ...
      for (var n = 0; n < iterations; n++) {
        ...
        // Shuffle and draw opponent + board cards
  ...
  function getHeroStreetActions(hand) {
    ...
    // Group all actions by street, track pot
    ...
    for (var si = 0; si < streetOrder.length; si++) {
      ...
      for (var ai = 0; ai < acts.length; ai++) {
        ...
        if (a.isMe && !heroAction && a.type !== 'won') {
          // First meaningful hero action on this street
      ...
      // If no non-blind action, check if hero posted blind
      if (!heroAction) {
  ...
  function generateGuidance(equity, streetInfo, streetName) {
    var eq = equity * 100;
  ...
  function runEquitySimulation(hand) {
    ...
    for (var i = 0; i < streetDefs.length; i++) {
      ...
      // Skip if board doesn't have enough cards for this street
      if (sd.boardSlice > board.length) break;

      // Skip streets after hero folded
      ...
      // Must have at least a blind action on preflop or some action
      if (!streetInfo && sd.name !== 'Preflop') continue;

      var sim = simulateStreet(heroHole, streetBoard, sd.iters);

      var guidance = streetInfo ? generateGuidance(sim.equity, streetInfo, sd.name) : { text: '', quality: 'neutral' };
  ...
  // ── Button injection ──────────────────────────────────────────────────────
  window.injectEquityButton = function(box, hand) {
    ...
    btn.onclick = function() {
      ...
      // Run async to let spinner paint
      setTimeout(function() {
