...
function renderAllIn(container, d, hands) {
  ...
  if (!_allinHands.length) {
    container.innerHTML =
      ...
      emptyState('No all-in showdown hands found yet. When you go all-in and both players show cards, those hands appear here with equity calculations.');
    return;
  }
  ...
  container.innerHTML =
    ...
    section('All-In Showdowns',
      `<div class="box text-center list">
        <div class="stat">
    ...
    section('Detected All-In Hands',
      `<div class="overflow-x"><table class="table">
        ...
        <tbody>${_allinHands.map(function(ah, i) { return _allinRow(ah, i, false); }).join('')}</tbody>
      </table></div>`);
...
function showAllInResults(container) {
  ...
  container.innerHTML =
    ...
    (cashAllIns.length >= 2
      ? chartSection('Cumulative All-In Results vs Expected Value (Cash Hands)', 'allin-ev-chart')
      : '') +
    (sum.variance ? emptyState(sum.variance) : '') +
    section('All-In Hand Details',
      (cashAllIns.length < allInHands.length
        ? '<div class="card text-micro allin-caveat">Side pots are approximated using total pot. Tournament hands are included in the table but excluded from the cumulative graph.</div>'
        : '') +
      `<div class="overflow-x"><table class="table">
        ...
        <tbody>${allInHands.map(function(ah, i) { return _allinRow(ah, i, true); }).join('')}</tbody>
      </table></div>`);
