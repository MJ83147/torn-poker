...
function _destroySessCharts() {
  for (var i = 0; i < _sessCharts.length; i++) {
    if (_sessCharts[i]) _sessCharts[i].destroy();
  }
  _sessCharts = [];
}
...
// A session sits at one table, so its big blind is that table's.
function _sessBB(session) {
  return session.tableId && TABLE_META[session.tableId] ? TABLE_META[session.tableId].bb : null;
}

function _sessListRow(s, i) {
  var tour = s.hands.some(function (h) {
    return !isCashHand(h);
  });
  var resCls = tour ? "c-muted" : s.pnl >= 0 ? "c-pos" : "c-neg";
  var resTxt = tour ? "Tournament" : fmtPnlBB(s.pnl, _sessBB(s));
...
function _sessStatCell(label, val, vs, cls) {
  return `<div class="stat"><div class="eyebrow">${label}</div>
    ...
    ${vs ? `<div class="text-meta">${vs}</div>` : ""}</div>`;
}
...
function _sessPhaseRow(label, ph, pnl, bb) {
...
function _renderSessionDetail(session, ctx, stories) {
  ...
  var bb = _sessBB(session);
  var resCls = tour ? "c-muted" : ctx.pnl >= 0 ? "c-pos" : "c-neg";
  var resTxt = tour ? "Tournament" : fmtPnlBB(ctx.pnl, bb);

  var peak = ctx.runline[0],
    low = ctx.runline[0];
  ...
  if (stories.length) {
    var cards = stories
      .map(function (st) {
        return typeof Sections !== "undefined" && Sections.renderStoryCard ? Sections.renderStoryCard(_sessStoryToFinding(st)) : "";
      })
      .join("");
  ...
  var n = ctx.hands.length,
    t = Math.floor(n / 3);
  function segPnl(a, e) {
    var s = 0;
    for (var k = a; k < e; k++) s += ctx.runline[k].pnl;
    return s;
  }
  html += dataTable({
    title: "By phase of the session",
    head: ["Phase", "Hands", "VPIP", "Aggr", "WTSD", "Net"],
    rows: [_sessPhaseRow("Early", ctx.phases.early, segPnl(0, t), bb), _sessPhaseRow("Middle", ctx.phases.mid, segPnl(t, 2 * t), bb), _sessPhaseRow("Late", ctx.phases.late, segPnl(2 * t, n), bb)],
  });
...
function _buildSessionCharts(ctx) {
  ...
  if (c1) {
    var stackData = ctx.runline.map(function (p) {
      return p.cum;
    });
    var stackLabels = ctx.runline.map(function (p) {
      return p.i;
    });
    _sessCharts.push(
      createChart(
          ...
          datasets: [
            {
              ...
              tension: 0.25,
              fill: true,
              backgroundColor: _sessGrad(c1, colors.gold),
              spanGaps: true,
            },
          ],
        },
        {
          ...
          scales: {
            x: chartXScale(colors, {
              ...
              maxTicksLimit: 8,
              maxRotation: 0,
              tickCallback: function (v, idx) {
                return "h" + this.getLabelForValue(v);
              },
            }),
          ...
          datasets: [
            {
              ...
              backgroundColor: function (c) {
                return c.raw >= 0 ? colors.green : colors.red;
              },
            },
          ],
        },
...
function _openSessionDetail(container, session, base) {
  ...
  var ctx = res.ctx;
  var buildWhenReady = function () {
    _buildSessionCharts(ctx);
  };
  if (typeof ensureChartJs === "function")
    ensureChartJs(function () {
      setTimeout(buildWhenReady, 0);
    });
...
function renderSessions(container, hands, meta, overallData) {
  ...
  if (listSlot)
    listSlot.innerHTML = dataTable({
      head: ["Date", "Table", "", "Hands", "Length", "Result", ""],
      rows: sessions.map(function (s, i) {
        return _sessListRow(s, i);
      }),
    });
