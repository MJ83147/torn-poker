...
function renderPosition(container, d, hands) {
  ...
  var rowsHtml = m.rows.map(function (r) {
    var deltaCell = "-";
    if (r.delta !== null) {
      var deltaStr = r.delta === 0 ? '<span class="c-pos">on target</span>' : `<span class="c-warn">${r.delta > 0 ? "+" : ""}${r.delta}%</span>`;
      deltaCell = `${deltaStr} <span class="text-meta">(${r.bandLo}-${r.bandHi}%)</span>`;
    }
    return `<tr class="link" data-pos-row="${r.pos}">
      ...
      <td>${deltaCell}</td>
      <td class="${pnlCls(r.pnl)}">${fmtPnlAgg(r.pnl, r.pnlBB)}</td>
  ...
  container.querySelectorAll("[data-pos-row]").forEach(function (row) {
    row.onclick = function () {
      ...
      var s = d.posMap[p];
      showExampleHandListModal(
        "Hands played from " + p,
        played,
        "Hands you played from " +
          p +
          ". Net P&L at this seat is " +
          fmtPnlAgg(s.pnl, s.pnlBB != null ? s.pnlBB : null) +
          " across " +
          s.hands +
          " hands. Look for the postflop pattern that repeats at this position.",
      );
    };
  });
      ...
      datasets: [
        {
          label: "Net P&L",
          data: m.chart.pnl,
          backgroundColor: m.chart.pnl.map(function (v) {
            return (v >= 0 ? colors.green : colors.red) + "99";
          }),
          borderColor: m.chart.pnl.map(function (v) {
