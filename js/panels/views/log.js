...
function renderLog(container, hands) {
  ...
  function renderLogPage() {
    ...
    var rows = m.pageHands.map(function(h, pi) {
...
function _logStarBtn(h, globalIdx) {
  ...
  return `<span class="btn btn-icon hrow-star${starred ? ' starred' : ''}" data-star-idx="${globalIdx}" title="${starred ? 'Unsave' : 'Save'} hand">${starred ? '&#9733;' : '&#9734;'}</span>`;
}
...
function renderSavedSection() {
  ...
  var cards = saved.map(function(x) {
    var h = x.entry.hand;
    var keyAttr = x.key.replace(/"/g, '&quot;');
    var notePreview = x.entry.note
      ? `<div class="text-micro saved-note-preview">${x.entry.note.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, ' ')}</div>`
      : '';
    return `<div class="card card-link list saved-card" data-saved-key="${keyAttr}">
      ...
      <div class="text-micro text-right">${fmtDateShort(x.entry.savedAt)}</div>
    </div>`;
  }).join('');
