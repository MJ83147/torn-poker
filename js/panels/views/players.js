...
function renderPlayers(container, d, hands) {
  ...
  function playerRow(o, maxH, isWatched) {
    ...
    return `<tr class="player-row link" data-player="${o.name}">
      ...
      <td class="${pnlCls(o.profit)}">${fmtPnl(o.profit)}</td>
    </tr>`;
  }
  ...
  function renderPlayerList() {
    if (!filtered.length) {
      container.innerHTML =
        ...
        emptyState('Not enough shared hands to show opponent stats. Keep playing to build data.');
      return;
    }
    ...
    if (watchedOpps.length) {
      watchedHtml = section('Watched Players',
        `<div class="list">
          <div class="text-micro">Click star to unwatch &middot; click row to view hands</div>
          <div class="overflow-x">
            <table class="table">
              <thead>${tableHead(['', 'Player', 'Hands', '', { tip: 'Win Rate' }, 'Net P&L'])}</thead>
              <tbody>${watchedOpps.map(function(o) { return playerRow(o, maxH, true); }).join('')}</tbody>
            </table>
          </div>
        </div>`);
    }
    ...
    var metaLine = searchFiltered.length + ' opponents' +
        ...
        <div class="list">
          ...
          <div>
            <table class="table">
              ...
              <tbody>${sortedOpps.map(function(o) { return playerRow(o, maxH, watched.indexOf(o.name) >= 0); }).join('')}</tbody>
            </table>
          </div>
        </div>`);
    ...
    function renderPage() {
      ...
      var ph = `<div class="player-detail">
        <div class="row"><div class="container">
          <div class="row between center wrap saved-section-divider">
            ...
            <div class="text-meta">${opp.hands} hands &middot; ${wr !== null ? wr + '% win' : '-'} &middot; ${fmtPnl(opp.profit)}</div>
          </div>
        </div></div>`;
...
function renderCompare(container, d, hands) {
  ...
  function renderComparison() {
    ...
    var rows = statRows.map(function(sr) {
      var v1 = s1[sr.key];
      var v2 = s2[sr.key];
    ...
    var exploitHtml = exploits.length
      ? `<div class="section-head">Exploit Tips</div><div class="list">${exploits.map(function(x) {
