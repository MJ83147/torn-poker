...
var _crCharts = [];
var _crPopover = null;
// Results only compute when the user presses Run report; any filter change
// invalidates the run so the report never re-runs behind their back.
var _crHasRun = false;
...
function _crClausePhrase(clauseId, segment) {
  var def = _crClauseDefs.find(function (c) {
    return c.id === clauseId;
  });
  if (!def) return null;
...
function _crRenderSentence(segment, segLabel) {
  ...
  for (var i = 0; i < segment.clauses.length; i++) {
    ...
    parts.push('<span class="sentence-token" data-segment="' + segLabel + '" data-clause="' + clauseId + '">' + displayPhrase + "</span>");
  }
  ...
  var available = _crClauseDefs.filter(function (c) {
    return segment.clauses.indexOf(c.id) === -1;
  });
  var addBtn = available.length ? ' <button class="sentence-add" data-segment="' + segLabel + '">+ add clause</button>' : "";
  sentence += addBtn;
...
function _crOpenAddClausePopover(targetEl, segLabel) {
  ...
  var hand = available.filter(function (c) {
    return c.kind === "hand";
  });
  var decision = available.filter(function (c) {
...
function _crOpenClausePopover(targetEl, segLabel, clauseId) {
  ...
  if (!def.options.length) {
    ...
    html += def.options
      .map(function (o) {
        ...
        return (
          '<label class="text-meta cr-pop-opt cr-pop-multi' +
          (checked ? " selected" : "") +
          '">' +
          ...
          "<span>" +
          o.label +
          (o.meta ? ' <span class="text-meta cr-pop-meta">' + o.meta + "</span>" : "") +
          "</span>" +
          "</label>"
        );
      })
    ...
    html += def.options
      .map(function (o) {
        ...
        return (
          '<button class="text-meta cr-pop-opt' +
          (sel ? " selected" : "") +
          '" data-val="' +
          o.value +
          '">' +
  ...
  _crShowPopover(targetEl, html, function (pop) {
    ...
    pop.querySelector(".cr-pop-remove").onclick = function () {
      segment.clauses = segment.clauses.filter(function (c) {
        return c !== clauseId;
      });
...
function _crRenderHeadline(result, compareResult) {
  ...
  function tile(label, val, cls) {
    return '<div class="box stat' + (dim ? " cr-tile-dim" : "") + '">' + '<div class="eyebrow">' + label + "</div>" + '<div class="value value-lg ' + cls + '">' + val + "</div>" + "</div>";
  }

  function tileCompare(label, valA, valB, clsA, clsB, delta, deltaCls) {
    return (
      '<div class="box stat cr-tile-compare">' +
      '<div class="eyebrow">' +
      label +
      "</div>" +
      '<div class="cr-tile-trio">' +
      '<div class="stat"><div class="eyebrow">A</div><div class="value ' +
      clsA +
      '">' +
      valA +
      "</div></div>" +
      '<div class="stat"><div class="eyebrow">Δ</div><div class="value ' +
      deltaCls +
      '">' +
      delta +
      "</div></div>" +
      '<div class="stat"><div class="eyebrow">B</div><div class="value ' +
      clsB +
      '">' +
      valB +
      "</div></div>" +
  ...
  function pnlCol(v) {
    return v == null ? "c-dim" : v >= 0 ? "c-pos" : "c-neg";
  }
  function deltaCol(v) {
    return v == null ? "c-dim" : v > 0 ? "c-pos" : v < 0 ? "c-neg" : "text-strong";
  }
  function wrCol2(v) {
  ...
  return (
    '<div class="cr-headline cr-headline-compare">' +
    ...
    tileCompare(
      "VPIP",
      m.vpip != null ? m.vpip + "%" : "-",
      mB.vpip != null ? mB.vpip + "%" : "-",
      "text-strong",
      "text-strong",
      m.vpip != null && mB.vpip != null ? (m.vpip - mB.vpip > 0 ? "+" : "") + (m.vpip - mB.vpip) + "%" : "-",
      "c-dim",
    ) +
    "</div>"
  );
...
function _crRenderInto(container) {
  ...
  var hasSelection = _crSegmentHasValue(_crState.A) || (_crState.compare && _crSegmentHasValue(_crState.B));
  ...
  if (!hasSelection) {
    ...
    headlineHtml =
      '<div class="row center cr-run-row">' +
      '<button class="btn btn-primary" id="cr-run-btn">Run report</button>' +
      '<div class="text-meta">Filters set. Run the report to see the numbers.</div>' +
      "</div>";
  } else {
    resultA = runCustomReport(_crHands, _crState.A, _crClauseDefs);
    resultB = _crState.compare ? runCustomReport(_crHands, _crState.B, _crClauseDefs) : null;
  ...
  container.innerHTML =
    ...
    section(
      "Report Builder",
      `<div class="row between cr-toolbar">
...
function _crRenderCharts(resultA, resultB) {
  ...
  if (trendCanvas && dataA.trend.length >= 2) {
    var labelsT = dataA.trend.map(function (p) {
      return p.label;
    });
    var datasets = [
      {
        label: dataB ? "A" : "bb/100",
        data: dataA.trend.map(function (p) {
          return p.bb100;
        }),
    ...
    if (dataB && dataB.trend.length >= 2) {
      var bSet = {};
      dataB.trend.forEach(function (p) {
        bSet[p.label] = p.bb100;
      });
      datasets.push({
        label: "B",
        data: labelsT.map(function (l) {
          return bSet[l] != null ? bSet[l] : null;
        }),
  ...
  if (posCanvas) {
    var active = POSITION_ORDER.filter(function (p) {
      return dataA.byPosition[p] && dataA.byPosition[p].bb100 != null;
    });
    if (active.length >= 2) {
      var posVals = active.map(function (p) {
        return dataA.byPosition[p].bb100;
      });
      var posDatasets = [
        {
          label: dataB ? "A" : "bb/100",
          data: posVals,
          backgroundColor: posVals.map(function (v) {
            return (v >= 0 ? colors.green : colors.red) + "99";
          }),
          borderColor: posVals.map(function (v) {
      ...
      if (dataB) {
        posDatasets.push({
          label: "B",
          data: active.map(function (p) {
            return dataB.byPosition[p] ? dataB.byPosition[p].bb100 : null;
          }),
      ...
      _crCharts.push(
        createChart(
          posCanvas,
          "bar",
          { labels: active, datasets: posDatasets },
          {
            legend: chartLegend(colors, !!dataB),
            tooltip: chartTooltip(colors, {
              label: function (c) {
  ...
  if (cardsCanvas) {
    ...
    var activeCls = classOrder.filter(function (c) {
      return dataA.byClass[c] && dataA.byClass[c].wr != null;
    });
    if (activeCls.length >= 2) {
      var labels = activeCls.map(function (c) {
        return dataA.byClass[c].label;
      });
      var vals = activeCls.map(function (c) {
        return dataA.byClass[c].wr;
      });
      var datasets = [
        {
          label: dataB ? "A" : "Win %",
          ...
          backgroundColor: vals.map(function (v) {
            return (v >= 50 ? colors.green : colors.red) + "99";
          }),
          borderColor: vals.map(function (v) {
            return v >= 50 ? colors.green : colors.red;
          }),
          borderWidth: 1,
          borderRadius: 4,
        },
      ...
      if (dataB) {
        datasets.push({
          label: "B",
          data: activeCls.map(function (c) {
            return dataB.byClass[c] ? dataB.byClass[c].wr : null;
          }),
          backgroundColor: colors.gold + "99",
          borderColor: colors.gold,
          borderWidth: 1,
      ...
      _crCharts.push(
        createChart(
          ...
          { labels: labels, datasets: datasets },
          {
            ...
            tooltip: chartTooltip(colors, {
              label: function (c) {
                return " " + c.parsed.y + "% win rate";
              },
            }),
            scales: {
              x: chartXScale(colors),
              y: chartYScale(colors, {
                max: 100,
                tickCallback: function (v) {
                  return v + "%";
                },
...
function renderCustomReport(container, hands) {
  ...
  ["A", "B"].forEach(function (seg) {
    ...
    _crState[seg].clauses = _crState[seg].clauses.filter(function (c) {
      return _crClauseDefs.some(function (def) {
        return def.id === c;
      });
    });
  });
